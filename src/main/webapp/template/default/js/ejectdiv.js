...
var url = null;
var content1="";
document.write('<div id="ejectDivC" style="z-index:1000;position:absolute;width:513px;" ><ul id="ejectDivUl" style="font-size:13px;padding:3px;list-style:none;min-width:200px;display:none;position:absolute;background:#fefefe;border:1px solid #cd9a5b;"></ul></div>');
document.write('<div id="ejectTableC" style="position:absolute;width:650px;border:1px solid #e6e6e6;display:none;" ></div>');
var $$ = function (id) {
...
ejectDiv.prototype.draw=function(){
	this.ajax();
	var content2;
	content2='<span style="display:inline-block;width:100%;"><input type="text" id="newtravel" style="border:1px solid #cd9a5b;width:142px;margin-bottom:5px;margin-right:0;padding:5px 3px;font-size:13px;" value=""/>&emsp;<input type="button" id="newtravelsub" value="添加新行程" style="border:0;line-height:27px;height:27px;margin:2px;padding:0;width:80px;color:#ffffff;background-color:#cd9a5b;font-size:12px;"/></span>';
...
ejectDiv.prototype.ajax=function() {
	if (window.XMLHttpRequest) {
		...
		url = "Bills.yy?a=btlid&date="+new Date();
		...
		if (req.status == 200) {
			...
			for(var key in TextDoc){
				for(var i=0;i<TextDoc[key].length;i++){
					content1+='<li data="'+TextDoc.type[i].bid+'" style="cursor: pointer; width:90px;padding:2px;margin:2px;color:#2b4490;border:1px solid #feeeed;">'+TextDoc.type[i].btypename+'</li>';
					}
				}
			}
		}
...
ejectDiv.prototype.ajaxadd=function() {
	...
	if (window.XMLHttpRequest) {
		req = new XMLHttpRequest();
		url = "Bills.yy?a=addtravel&btype="+$$("newtravel").value+"&date="+new Date();
	} else if (window.ActiveXObject) {
		req = new ActiveXObject("Microsoft.XMLHTTP");
		url = "Bills.yy?a=addtravel&btype="+$$("newtravel").value+"&date="+new Date();
	}
...
displaydetail.prototype.ajax=function() {
	...
	if (window.XMLHttpRequest) {
		req = new XMLHttpRequest();
		url = "Bills.yy?a=btmid&btid="+this.objdata.getAttribute("data")+"&date="+new Date();
	} else if (window.ActiveXObject) {
		req = new ActiveXObject("Microsoft.XMLHTTP");
		url = "Bills.yy?a=btmid&btid="+this.objdata.getAttribute("data")+"&date="+new Date();
	}
		...
		if (req.status == 200) {
			...
			for(var key in TextDoc){
				for(var i=0;i<TextDoc[key].length;i++){
					...
					content1+='<div class="split" id=""> <div class="weibo" data-tid="" data-login="" data-uid="" data-huifu=""> <div class="wb_l"> <div> <img class="author" src="'+avatar+'" onclick="goToUserInfo(2);" /> </div> </div> <div class="wb_r"> <div class="user_info"> <span class="fl p_u">'+TextDoc.type[i].username+'</span> <span class="fr p_t"> <span>'+TextDoc.type[i].bdate.substring(0,19)+'</span> </span> </div> <div class="wb_c_wp"> <div class="wb_c">由于 '+TextDoc.type[i].btypename +b+'<span class="wordColor">人民币'+c+'元。</span> </div> <div class="from"> <span class="fl">'+TextDoc.type[i].bcaption+'</span> <span class="fr num"> </span> </div> ';
...
/**显示可以选择的账单类型*/
document.write('<div id="ejectBtypeDivC" style="position:absolute;width:513px;" ><ul id="ejectBtypeDivUl" style="font-size:13px;padding:3px;list-style:none;min-width:200px;display:none;position:absolute;background:#fefefe;border:1px solid #cd9a5b;"></ul></div>');
...
ejectBtypeDiv.prototype.draw=function(){
	this.ajax();
	var content2;
	content2='<span style="display:inline-block;width:100%;"><input type="text" id="newbtype" style="border:1px solid #cd9a5b;width:142px;margin-bottom:5px;margin-right:0;padding:5px 3px;font-size:13px;" value=""/>&emsp;<input type="button" id="newbtypesub" value="添加新类型" style="border:0;line-height:27px;height:27px;margin:2px;padding:0;width:80px;color:#ffffff;background-color:#6d8346;font-size:12px;"/></span>';
	this.contents.innerHTML=content1+content2;
};
...
ejectBtypeDiv.prototype.complete=function(){
	if (req.readyState == 4) {
		if (req.status == 200) {
			...
			for(var key in TextDoc){
				for(var i=0;i<TextDoc[key].length;i++){
					content1+='<li data="'+TextDoc.type[i].btypeid+'" style="float:left;cursor: pointer; width: 90px;padding:2px;margin:2px;color:#2b4490;border:1px solid #feeeed;">'+TextDoc.type[i].btypename+'</li>';
					}
				}
			}
		}
...
function displayorgimg(imgpath){
	...
	for(var i = 0; i < locateimg_img_list.length; i++) {
		...
		obj.onload=function(){
			...
			(obj.height*1-window.screen.availHeight*1)<0?t.style.top=(window.screen.availHeight*1-obj.height*1)/5+"px":t.style.top=0+"px";
		}
	}
}
