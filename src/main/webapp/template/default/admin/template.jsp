...
<c:forEach items="${list}" var="list" varStatus="status">
	...
	<tr <c:if test="${status.index%2!=0}">class="split"</c:if>>
          ...
          <td class="ttype"><select name="ttype" disabled><option value="0"<c:if test="${list.ttype==0}">selected</c:if>>电脑</option><option value="1"<c:if test="${list.ttype==1}">selected</c:if>>手机</option></select></td>
          <td><c:if test="${list.ttype==0}"><a href="Admin.yy?a=template&operate=setdefault&tid=${list.tid}&tdirectory=${list.tdirectory}">默认</a></c:if>
        <c:if test="${list.ttype==1}"><a href="Admin.yy?a=template&operate=setmobiledefault&tid=${list.tid}&tdirectory=${list.tdirectory}">默认</a></c:if>  
          <a href="Admin.yy?a=template&operate=delete&tid=${list.tid}" onclick="if(confirm('删除后不可恢复！！！')){return true;}return false;">删除</a>
         </form> </td>
      </tr></c:forEach>
      <form method="post" action="<%=basePath%>Admin.yy"><tr><td></td><td></td><td><input type="text" name="tname"/></td><td><input type="text" name="tdirectory"/></td><td><input type="text" name="tcopyright"/></td><td><select name="ttype"><option value="0">电脑</option><option value="1">手机</option></select></td><td><input type="submit" value="添加"/><input type="hidden" name="a" value="template"/><input type="hidden" name="operate" value="add"/></td></tr></form>
</table>
</body>
</html>
